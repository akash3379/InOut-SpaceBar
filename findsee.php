  ...
  <section class="top-bar-section">
    ...
    <div class="row">
      <div class="large-12 columns">
		...
		$sql = "SELECT * FROM `data` where `username`='$user' AND `id`='$id'";
			$result = mysql_query($sql);
			while($row = mysql_fetch_array($result))
			{
			?>
			<center><h3><u><b><?php echo $row['trip_name']; ?></b></u></h3></center>
			<h2> <?php echo $row['from']; ?> to <?php echo $row['to']; ?> in only <?php echo $row['total']; ?>.
    ...
    <?php } ?>
		...
		<h3>Decription:</h3><br>
		<?php
			$sql = "SELECT dsc FROM `data` where `username`='$user' AND `id`='$id'";
			$result = mysql_query($sql);
